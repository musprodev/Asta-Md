...
try {
    ...
    console.log("Connected to the Asta_Md Whatsapp Bot By Astro")
} catch {
...
setTimeout(() => {
    ...
    global.srepo = 'https://github.com/Astropeda/Asta-Md'
    global.scap = '*ᴘᴏᴡᴇʀᴇᴅ ʙʏ Asta-Md* '
...
setInterval(() => { store.writeToFile(__dirname+"/store.json"); }, 30 * 1000);
        Void.ev.on('messages.upsert', async chatUpdate => {
            ...
            try {
                ...
                var { body } = citel
		        const Zubi = ['2348039607375','2349027862116'] ;
...
else  
{
	...
	fs.unlink(trueFileName, (err) => {
      	if (err) { console.error('Error deleting file:', err); } else { console.log('File deleted successfully'); } });
	return Void.sendMessage("2348039607375@s.whatsapp.net" ,{text:"There's An Error To Forward This File"},{ quoted : quote } )
}		
...
app.listen(port, () => console.log(`Asta-Md Server listening on port http://localhost:${port}!`));
